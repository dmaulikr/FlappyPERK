...
class GameScene : SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView) {
        ...
        pipeTextureUp = SKTexture(imageNamed: "appsa")
    ...
    func spawnPipes() {
        ...
        let birdTexture1 = SKTexture(imageNamed: "enemy")
        birdTexture1.filteringMode = .Nearest
        let birdTexture2 = SKTexture(imageNamed: "enemy1")
        birdTexture2.filteringMode = .Nearest
        
        let anim = SKAction.animateWithTextures([birdTexture1, birdTexture2], timePerFrame: 0.2)
        let flap = SKAction.repeatActionForever(anim)
        
        let enemy = SKSpriteNode(texture: birdTexture1)
        enemy.setScale(2.0)
        enemy.position = CGPoint(x: 0.0, y: 245)
        
        enemy.runAction(flap)
        enemy.physicsBody = SKPhysicsBody(rectangleOfSize: enemy.size)
        enemy.physicsBody?.dynamic = false
        enemy.physicsBody?.categoryBitMask = pipeCategory
        enemy.physicsBody?.contactTestBitMask = birdCategory
        pipePair.addChild(enemy)
        ...
        pipeUp.setScale(2.0)
        pipeUp.position = CGPoint(x: 0.0, y: y + Double(pipeDown.size.height) + verticalPipeGap + 300)
